...
async function get_antiforgerystuff() {
    let r = await do_request(settingslist.login_url);
    if (r.ok) {
...
(async() => {
    ...
    let res = await do_request(settingslist.auth_url,
        "POST", {
            cookie: af[0],
    ...
    let fid = await get_fid(
        await do_request(settingslist.calendar_url, "GET", { cookie: `${af[0]};${get_lc(res)}` }, `vt=month`)
    );
    console.log("Extracting calendar data...");
    res = await do_request(settingslist.response_url, "POST", { cookie: `${af[0]};${get_lc(res)}` }, `start=${moment().format("YYYY-MM-DD")}&end=${moment().add(1, "year").format("YYYY")}-12-31&resType=104&calView=month&federationIds%5B%5D=${fid}`);
